...
// InitializeRooms and other methods remain the same...

// InitializeRooms creates all rooms for a city on a planet.
func InitializeRooms() map[string]*Room {
	// Create rooms for a city
	...
	// Assume additional rooms that can be entered or exited
	secretLab := NewRoom("Secret Lab", "A hidden laboratory with advanced technology.", nil, nil, nil, nil, nil, nil)
	armory := NewRoom("Armory", "A secure room where weapons are stored.", nil, nil, nil, nil, nil, nil)

	// Link enter and exit for specific rooms
	townSquare.Enter = secretLab
	secretLab.Exit = townSquare

	barracks.Enter = armory
	armory.Exit = barracks
